...
interface OrganizationsState {
  ...
  selectedOrgApps: ApplicationOrg[];
...
export const organizationsSlice = createSlice({
  ...
  reducers: {
    ...
    setOrganization: (state, action: PayloadAction<Organization>) => {
      ...
      state.selectedOrgApps = action.payload.applications;
    },
    resetOrganization: (state) => {
      state.organization = initialState.organization;
      state.selectedOrgApps = [];
    },
    setOrgFormModified: (state, action: PayloadAction<boolean>) => {
      state.orgFormModified = action.payload;
    },
    updateOrgApplication: (state, action: PayloadAction<ApplicationOrg[]>) => {
      state.selectedOrgApps = action.payload;
  },
  },
...
export const selectedOrgApplications = (state: RootState) =>
state.organizations.selectedOrgApps;
...
export const postLogoutRedirectUrl = (state: RootState) =>
  ...
  export const selectOrgIDByDomain = (state: RootState, email: string) => {
  ...
  export const checkIfValidDomain = (state: RootState, email: string) => {
    //strip email part and do domain
    ...
    if(org?.id && (emailPart !== '')){return true} else {return false};
  };
...
export const getAppsByOrgCode = (state: RootState, orgCode: string) => {
  const org = state.organizations.organizations.find((p) =>
    p.orgCode?.includes(orgCode)
  );
  return org?.applications ? org.applications.map((app) => 
     app.appCode) : [];
};

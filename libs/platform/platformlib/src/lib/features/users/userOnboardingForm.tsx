...
import { Alert, AlertColor, Box, Button, Card, FormControl, Grid, InputLabel, List, ListItem, Typography } from '@mui/material';
import {getOrgCodeFromName, organizationList}  from '../../../../../../redux-store/src/lib/Platfotm-UI/organizations/organizationsSlice';
import { useAppDispatch, useAppSelector } from "../../../../../../redux-store/src/lib/hooks";
...
import { importUserFile } from '../../../../../../redux-store/src/lib/Platfotm-UI/users/userSlice';
...
const options: MUIDataTableOptions = {
  ...
  setRowProps: (data, dataIndex, rowIndex) => {
    if (data[3].length > 0) {
      return {
        //#D32F2F
        style:{backgroundColor: rowIndex % 2 === 0 ? "rgba(211, 47, 47, 0.4)" : "rgba(211, 47, 47, 0.3)"}
      };
    }
    else{
      return {
        ...
        dispatch(importUserFile({
          ...
          .then(
            (value) => {
              ...
              if((res.invalidUsers + res.validUsers === res.readUsers) && res.dataUpload === true){
                setAlert(true);
...
return (
  ...
  response ?
      <Grid container >
        <UnsavedData
          ...
          location="onboarding"
        />
      <TitleAndCloseIcon breadCrumbOrigin="ALL USERS" breadCrumbTitle="Import Users" onClickButton={backToUpload} ></TitleAndCloseIcon>
                ...
                <Typography
                    ...
                    <Grid container item>
                      ...
                      <Grid item xs = {4} sx={{}}>   
                        {
                          response.dataUpload ? 
                          <Typography  style={{paddingLeft:"30%", color:"#1B5E20", textAlign:"left"}}>
                              Valid users uploaded
                          </Typography> 
                          :
                          <Typography style={{paddingLeft:"30%", color:"#C62828",textAlign:"left"}}>
                              No users uploaded
                          </Typography> 
                        }  
                      </Grid>
                      ...
                      <Grid  item xs={4}></Grid>
                      <Grid item xs = {4}>
                            <Typography style={{ paddingLeft:"30%",textAlign:"left"}} >
                      ...
                      <Grid  item xs={4} > </Grid>
                      <Grid item xs = {4} >
                            <Typography color="#C62828" style={{paddingLeft:"30%",textAlign:"left"}}>
                                Invalid Users: {response.invalidUsers}
                            </Typography>  

                            <Typography color="#1B5E20" style={{paddingLeft:"30%", textAlign:"left"}}>
                      ...
                      <Grid  item xs={4} > </Grid>
                            ...
                            <Grid item xs = {8}> 
                                        <MUIDataTable
                                          title={""}
                                          data={badRecords.concat(goodRecords).sort((a, b)=> a.RowNumber < b.RowNumber ? -1 : a.RowNumber > b.RowNumber ? 1 : 0)}
                                          columns={columns}
                                          options={options}
                                        />
                            </Grid>
  ...
  <Grid container>
    <UnsavedData
          ...
          location="onboarding"
        />
    <TitleAndCloseIcon breadCrumbOrigin="ALL USERS" breadCrumbTitle="Import Users" onClickButton={backToInstructions} ></TitleAndCloseIcon>
    {snackbar && <Snackbar type={snackbarType} content={snackBarMsg} />}
  <Grid item xs={12}>
      ...
      <Grid py={4} px={2}>
              ...
              <Grid container item  sx={{ marginBottom: "10%"}}>
               <Grid
                  item
                  xs={12}
                  sx={{ alignSelf:"center", textAlign: "center", alignContent:"center"}}
                  > 
                  <div style={{display:'flex', justifyContent:"center", flexDirection:'row', alignItems:'center'}}>
                   <InputLabel style={{ marginTop:'25px', marginRight:'10px' , color:"#6513F0"}} id="custom-select">
                      {"Organization"}
                    </InputLabel>
                  ...
                  <ImportFile 
                      ...
                      org={org}
                      fileName={fileName}
                      onImportFileUpdate={handleImportFileUpdate}></ImportFile>
                  
                  <Button
                      disabled={(fileName.length > 0) && (org.length > 0) ? false: true}
                      variant="outlined"
                      sx={{
                        marginRight: theme.spacing(2),  
                      }}
                  ...
                  {alertContent.length >0 ?<><Typography>Error processing file, please review. Common issues are listed below</Typography>
                  <List sx={{ paddingLeft:"35%", alignContent:"center"}}>
                                <ListItem style={{alignContent:"center"}}>Empty Column Values</ListItem>
          ...
          <LowerButton buttonName="BACK" onClickButton={backToInstructions}></LowerButton>              
      </Grid>
  </Grid>
  </Grid>
  }

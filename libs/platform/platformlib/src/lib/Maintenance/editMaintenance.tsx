...
import { Box, IconButton, Typography, Button, Grid, TextField, FormControlLabel, Checkbox } from "@mui/material";
...
const EditMaintenanceMode = (props: Props) => {
  ...
  const  token  =    oktaAuth?.getAccessToken();
    ...
    const handleAppChange = (updatedList: App[]) => {
            ...
            dispatch(createMaintenance(
                {
                    ...
                    token: token,
                    data: {
                        maintenanceStartDate: moment(maintenanceStartDate).tz('est').format("MM-DD-YYYY HH:mm"),
                        maintenanceEndDate: moment(maintenanceEndDate).tz('est').format("MM-DD-YYYY HH:mm"),
                ...
                <DialogContent>
                    <DialogContentText id="alert-dialog-description" component={"div"}>
                        <Grid container spacing={1.5} sx={{ paddingY: "5px" }}>
                            <Grid item xs={2.4}>
                                <Typography sx={{ color: `${theme.palette.text.primary}`, paddingY: "15px" }}>
                                    Start
                                </Typography>
                            </Grid>
                            <Grid item xs={9}>
                                <DateAndTimeInput
                                    ...
                                    handleMaintStartDate={(value) => handleMaintanenceStartDate(value)}
                            ...
                            <Grid item xs={9}>
                                <DateAndTimeInput
                                    ...
                                    minDateAndTime={parseISO(moment(maintenanceStartDate).format())}
                                    handleMaintEndDate={(value) => handleMaintanenceEndDate(value)}
                                    invalidDate={(value: boolean) => handleInvalidDate(value)}
                                />
                            </Grid>
                            <Grid item xs={2.4}>
                            ...
                            <Grid item xs={2.4}>
                                <Typography sx={{ color: `${theme.palette.text.primary}`, paddingY: "20px" }}>
                                    Reason
                                </Typography>
                            </Grid>
                            <Grid item xs={9} >
                            ...
                            <Grid item xs={9}>
                                <DateAndTimeInput
                                    ...
                                    handleAnnounceStartDate={(value) => handleAnnouncementStartDate(value)}
                                    invalidDate={(value: boolean) => handleInvalidDate(value)}
                                    maxDateAndTime={parseISO(moment(maintenanceStartDate).add(1, 'm').format())}
                                />
                            </Grid>
                        </Grid>
                    </DialogContentText>
